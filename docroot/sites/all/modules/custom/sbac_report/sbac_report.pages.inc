...
function sbac_report_individual() {
  ...
  $csv_form = drupal_get_form('sbac_report_csv_form');
  $csv_tab = theme('sbac_report_csv', array(
      'form' => $csv_form,
    )
  );
...
function sbac_report_build_summation_table($uid, $date_range, $display = 5) {
  ...
  foreach ($result as $row) {
    ...
    // resource title
    $item[] = array(
      'data' => l($row->node_title, 'node/' . $row->nid),
      'class' => 'cell title',
    );
    ...
    // review date
    $item[] = array(
      'data' => date('m/d/y', $row->review_date),
      'class' => 'cell date',
    );
...
function sbac_report_individual_modal_form($form, &$form_state, $node, $state, $trigger_id, $uid) {
  ...
  // default state
    ...
    $date = '';
    if ($user_review) {
      $date = t('Review Date: !date', array('!date' => date('m/d/y', $user_review->created)));
          ...
          $qc_rec_slide .= '
                            <div class="qc-item">
                              <div class="qc-rec-name">
                                ' . theme('username', array('account' => $qc_account)) . $location . '
                              </div>
    ...
    // add Quality Criteria question slides
      if ($qc_entities) {
        ...
        for ($i = 0; $i < $questions_count; $i++) {
          $slide = '<div class="slide-title">' . t('Quality Criteria !count', array('!count' => $i + 1)) . '</div>';
